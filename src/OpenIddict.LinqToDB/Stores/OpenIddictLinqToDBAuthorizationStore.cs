...
namespace OpenIddict.LinqToDB
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public virtual async ValueTask DeleteAsync(TAuthorization authorization, CancellationToken cancellationToken)
        {
            ...
            {
                await Context.DeleteAsync(token, token: cancellationToken);
            }

            await Context.DeleteAsync(authorization, token: cancellationToken);
            ...
            string subject, string client, CancellationToken cancellationToken)
        {
            ...
            return (from authorization in Authorizations.LoadWith(authorization => authorization.Application)
            ...
            ImmutableArray<string> scopes, CancellationToken cancellationToken)
        {
            ...
            async IAsyncEnumerable<TAuthorization> ExecuteAsync([EnumeratorCancellation] CancellationToken cancellationToken)
            {
                ...
                var authorizations = (from authorization in Authorizations.LoadWith(authorization => authorization.Application)
                                      where authorization.Subject == subject &&
                                            authorization.Status == status &&
                                            authorization.Type == type
                                      join application in Applications on authorization.Application!.Id equals application.Id
        ...
        public virtual async ValueTask<TAuthorization?> FindByIdAsync(string identifier, CancellationToken cancellationToken)
        {
            ...
            return await (from authorization in Authorizations.LoadWith(authorization => authorization.Application)
        ...
        public virtual async ValueTask<string?> GetApplicationIdAsync(TAuthorization authorization, CancellationToken cancellationToken)
        {
            ...
            //TODO: Connect application to authorization inner join
            //// If the application is not attached to the authorization, try to load it manually.
            //if (authorization.Application is null)
            //{
            //    var reference = authorization.Application .Entry(authorization).Reference(entry => entry.Application);
            ...
            TState state, CancellationToken cancellationToken)
        {
            ...
            return await query(
                Authorizations.LoadWith(authorization => authorization.Application), state).FirstOrDefaultAsync(cancellationToken);
        }
        ...
        {
            var query = Authorizations.LoadWith(authorization => authorization.Application)
                                      .OrderBy(authorization => authorization.Id!).AsQueryable();
            ...
            TState state, CancellationToken cancellationToken)
        {
            ...
            return query(
                Authorizations.LoadWith(authorization => authorization.Application), state).AsAsyncEnumerable();
        }
        ...
        public virtual async ValueTask PruneAsync(DateTimeOffset threshold, CancellationToken cancellationToken)
        {
            ...
            for (var index = 0; index < 1_000; index++)
            {
                ...
                var authorizations =
                    await (from authorization in Authorizations.LoadWith(authorization => authorization.Tokens)
                ...
                await Context.DeleteAsync(authorizations, token: cancellationToken);
