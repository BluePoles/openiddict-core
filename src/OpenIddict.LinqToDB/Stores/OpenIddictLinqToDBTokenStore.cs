...
namespace OpenIddict.LinqToDB
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public virtual IAsyncEnumerable<TToken> FindAsync(string subject, string client, CancellationToken cancellationToken)
        {
            ...
            return (from token in Tokens.LoadWith(token => token.Application).LoadWith(token => token.Authorization)
        ...
        public virtual async ValueTask<TToken?> FindByIdAsync(string identifier, CancellationToken cancellationToken)
        {
            ...
            return await (from token in Tokens.LoadWith(token => token.Application).LoadWith(token => token.Authorization)
            ...
            TState state, CancellationToken cancellationToken)
        {
            ...
            return new ValueTask<TResult>(query(
                Tokens.LoadWith(token => token.Application)
                      .LoadWith(token => token.Authorization), state).FirstOrDefaultAsync(cancellationToken));
        }
        ...
        {
            var query = Tokens.LoadWith(token => token.Application)
                              .LoadWith(token => token.Authorization)
                              .OrderBy(token => token.Id!).AsQueryable();
            ...
            TState state, CancellationToken cancellationToken)
        {
            ...
            return query(
                Tokens.LoadWith(token => token.Application)
                      .LoadWith(token => token.Authorization), state).AsAsyncEnumerable();
        }
        ...
        public virtual async ValueTask PruneAsync(DateTimeOffset threshold, CancellationToken cancellationToken)
        {
            ...
            //TODO: Implement linq2db transaction
            ...
            for (var index = 0; index < 1_000; index++)
            {
                ...
                await Context.DeleteAsync(tokens, token: cancellationToken);
